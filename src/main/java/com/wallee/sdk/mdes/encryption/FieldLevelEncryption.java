...
public class FieldLevelEncryption {
    ...
    public static String encryptPayload(String payload, FieldLevelEncryptionConfig config, FieldLevelEncryptionParams params) throws EncryptionException {
        try {
            ...
            for (Entry<String, String> entry : config.getEncryptionPaths().entrySet()) {
    ...
    public static String decryptPayload(String payload, FieldLevelEncryptionConfig config, FieldLevelEncryptionParams params) throws EncryptionException {
        try {
            ...
            for (Entry<String, String> entry : config.getDecryptionPaths().entrySet()) {
                String jsonPathIn = entry.getKey();
                String jsonPathOut = entry.getValue();
    ...
    private static void encryptPayloadPath(DocumentContext payloadContext, String jsonPathIn, String jsonPathOut,
        ...
        String encryptedValue = encodeBytes(encryptedValueBytes, config.getFieldValueEncoding());
        ...
        payloadContext.put(jsonPathOut, config.getEncryptedValueFieldName(), encryptedValue);
        if (!isNullOrEmpty(config.getIvFieldName())) {
            payloadContext.put(jsonPathOut, config.getIvFieldName(), params.getIvValue());
        }
        if (!isNullOrEmpty(config.getEncryptedKeyFieldName())) {
            payloadContext.put(jsonPathOut, config.getEncryptedKeyFieldName(), params.getEncryptedKeyValue());
        }
        if (!isNullOrEmpty(config.getEncryptionCertificateFingerprintFieldName())) {
            payloadContext.put(jsonPathOut, config.getEncryptionCertificateFingerprintFieldName(), config.getEncryptionCertificateFingerprint());
        }
        if (!isNullOrEmpty(config.getEncryptionKeyFingerprintFieldName())) {
            payloadContext.put(jsonPathOut, config.getEncryptionKeyFingerprintFieldName(), config.getEncryptionKeyFingerprint());
        }
        if (!isNullOrEmpty(config.getOaepPaddingDigestAlgorithmFieldName())) {
            payloadContext.put(jsonPathOut, config.getOaepPaddingDigestAlgorithmFieldName(), params.getOaepPaddingDigestAlgorithmValue());
        }
    }
    ...
    private static void decryptPayloadPath(DocumentContext payloadContext, String jsonPathIn, String jsonPathOut,
        ...
        Object encryptedValueJsonElement = readAndDeleteJsonKey(payloadContext, jsonPathIn, inJsonObject, config.getEncryptedValueFieldName());
        ...
        if (params == null) {
            ...
            Object oaepDigestAlgorithmJsonElement = readAndDeleteJsonKey(payloadContext, jsonPathIn, inJsonObject, config.getOaepPaddingDigestAlgorithmFieldName());
        ...
        // Decrypt data
        byte[] encryptedValueBytes = decodeValue(jsonEngine.toJsonString(encryptedValueJsonElement), config.getFieldValueEncoding());
